...
function colors._highlight(group, properties, ns)
    ...
    if properties.style then
        if type(properties.style) == "table" then
            ...
            if not vim.tbl_contains({ "none", "NONE", "None" }, properties.style) then args[properties.style] = true end
        end
    end
    if properties.sp then args["sp"] = properties.sp end
...
function colors.set(highlights, scheme)
    highlights = highlights or "builtin"
    scheme = scheme or art.config.colorscheme
    if vim.loop.fs_stat(vim.fn.stdpath("config") .. "/lua/art/colors/themes/" .. scheme .. ".lua") then
        ...
        callback = function()
            vim.schedule(function() require("art.colors.external.kitty").set() end)
        end
    else
