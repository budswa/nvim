...
local components = {
    ...
    mode = {
        ...
        provider = function(self) return self.modes[self.mode][1] end,
        hl = function(self) return { fg = self.modes[self.mode][2] or colors.b09, bg = colors.b01, bold = true } end,
    },
    ...
    buffer = {
        ...
        {
            provider = function()
                if not vim.bo.modifiable and vim.bo.readonly then return ' [X] ' end
                return ''
            end,
            hl = { fg = colors.b12 },
        },
    ...
    cwd = {
        ...
        hl = function() return { fg = colors.b09, bold = true } end,
    },

    -- time - time and uptime
    time = {
        init = function(self) self.starttime = os.time() end,
        provider = function(self)
    ...
    diagnostics = {
        ...
        {
            provider = function(self)
                if self.errors > 0 then return 'E: ' .. self.errors end
            end,
            hl = 'DiagnosticError',
        },
        {
            provider = function(self)
                if self.warnings > 0 then return 'W: ' .. self.warnings end
            end,
            hl = 'DiagnosticWarn',
        },
        {
            provider = function(self)
                if self.info > 0 then return 'I: ' .. self.info end
            end,
            hl = 'DiagnosticInfo',
        },
        {
            provider = function(self)
                if self.hints > 0 then return 'H: ' .. self.hints end
            end,
            hl = 'DiagnosticHint',
        },
    },
...
local statusline = {
    ...
    -- Default
    {
        hl = function() return { fg = colors.b06, bg = colors.b01 } end,
        components.space,
        components.mode,
        --components.space,
        --components.buffer,
