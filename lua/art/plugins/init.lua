...
local plugins = {
	...
	},
	{
		...
		setup = function()
			vim.schedule(function()
				require("packer").loader("modes.nvim")
			end)
		end,
		config = function()
			require("modes").setup({})
	...
	},
	{
		...
		config = function()
			require("clangd_extensions").setup({
				server = {
					on_attach = require("art.modules.lsp").on_attach,
					capabilities = require("art.modules.lsp").capabilities,
					cmd = {
						"clangd",
						"-j=8",
						"--background-index",
	...
	---- Completion
	{
		...
		config = function()
			require("art.plugins.completion")
		end,
	},
    ...
    },
	{
		...
		cond = function()
			if vim.loop.fs_stat(vim.fn.stdpath("config") .. "/.editorconfig") then
				return true
			end
		end,
	},
