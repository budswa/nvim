...
local function set_option(option, default)
	...
	if type(M.options[option]) == "boolean" then
		M.options[option] = M.options[option] and 1 or 0
	end

	-- Set option to either the option value or the default
	M.options[option] = M.options[option] or split_on_comma(vim.g[option]) or default
end
...
local set_cursor_position = function()
	...
	if fn.foldclosed(".") ~= -1 and M.options.lastplace_open_folds == 1 then
		vim.api.nvim_command([[normal! zvzz]])
	end
end

function M.lastplace_buf()
	-- Check if the buffer should be ignored
	if vim.tbl_contains(M.options.lastplace_ignore_buftype, vim.api.nvim_buf_get_option(0, "buftype")) then
		return
	end

	if fn.has('nvim-0.5.1') == 1 then
		-- Check if the filetype should be ignored
		if vim.tbl_contains(M.options.lastplace_ignore_filetype, vim.api.nvim_buf_get_option(0, "filetype")) then
